...
| 2026-09-10 | **LM4 电磁波持续学习启动** | 新方向: 把 headless 持续学习框架扩展到 **NASA CDAWeb 真实空间物理电磁波数据**。数据: Van Allen Probes RBSP-A EMFISIS(`DENSITY_EMFISIS-L4` 6s + `MAGNETOMETER_1SEC-GEI_EMFISIS-L3`), 2015 全年 DENSITY(408万点)+ Q1 MAG(130万点), 对齐后 **197,708 窗口**(窗口32)。**关键设计**: ① 排除 `fpe`/`fuh`/`wpe_over_wce`(与 density 有解析关系 = 标签泄漏), 只用 `Magnitude`/`rms`(波活动)/`lambda`/`delta`; ② 物理状态 = log10(density) 跨 6.3 数量级分 6 个分位数域(各 ~33,000 样本); ③ 时间对齐必须近邻匹配(精确时间戳全失败)。**可解性诊断**: 联合训练上限 6类 0.523 / 3类 0.720(随机 0.167/0.333)→ 任务可解。**发现**: 服务器直连 CDAWeb 慢 17×(67s vs 本机 4s/天)→ 数据本机下载后传输。文档: `docs/lm4_wave.md`, 脚本: `tests/{dl_local,dl_mag,run_lm4_wave}.py`。 |