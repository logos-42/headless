...
| 2026-09-06 | **LM1 因果持续学习对照 — ⚖️ 诚实: 纯反馈因果提议器无稳定增益, value 反超** | `tests/run_lm1_causal.py`, 把真实 unseen acc 每轮回传给 `CausalProposer` (observe)。三类规模: 小(2轮) causal +0.0065 略胜; 中(4轮,d64) value [0.0794] > causal [0.0612](βf0.6) 与 [0.0664](βf3.0)。机制原因: 探索仅占训练一小部分(信号稀释) / 反馈回合滞后 / causal 覆盖不如 value 频率惩罚均匀。**结论: 算法启发式有效≠集成进 RL 回路有效, credit assignment 需更在线**。`run_lm1_production.py` 加 `--proposer {value,causal}` + `_after_evaluate` hook。`docs/wiki/causal_lm1_experiment.md` |
